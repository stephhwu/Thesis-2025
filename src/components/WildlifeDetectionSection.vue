    ...
    setup() {
      ...
      const drawChart = () => {
        ...
        const dogPoints = g.selectAll(".dog-point")
          ...
          .attr("xlink:href", "/src/assets/images/daisy.png") // Use absolute path from root or relative from public
          .attr("x", d => x(d) - 40) // Center image (adjust size)
          .attr("y", d => yScales[d](dogData[d]) - 40) // Center image (adjust size)
          .attr("width", 90)
          .attr("height", 90)
          .style("opacity", 0)
          .on("mouseover", (event, d) => showTooltip(event, d, dogData, "Daisy"))
          .on("mousemove", (event) => {
